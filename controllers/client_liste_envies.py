...
def client_liste_envies_show():
    mycursor = get_db().cursor()
    id_client = session['id_user']
    vetements_liste_envies = []
    vetements_historique = []
    return render_template('client/liste_envies/liste_envies_show.html'
                           ,vetements_liste_envies=vetements_liste_envies
                           , vetements_historique=vetements_historique
                           #, nb_liste_envies= nb_liste_envies
                           )



def client_historique_add(vetement_id, client_id):
    mycursor = get_db().cursor()
    client_id = session['id_user']
    # rechercher si l'vetement pour cet utilisateur est dans l'historique
    # si oui mettre
    sql ='''   '''
    mycursor.execute(sql, (vetement_id, client_id))
...
def client_liste_envies_vetement_move():
    mycursor = get_db().cursor()
    id_client = session['id_user']
    id_vetement = request.args.get('id_vetement')
